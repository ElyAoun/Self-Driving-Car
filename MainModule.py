...
def drive():
    ...
    sleep(0.1)
    fourcc = cv2.VideoWriter_fourcc(*'XVID')
    out1 = cv2.VideoWriter('raw_feed_1.avi', fourcc, 30.0, (int(pi_cam.camera.get(3)), int(pi_cam.camera.get(4))))
    out2 = cv2.VideoWriter('lanes_feed_1.avi', fourcc, 30.0, (int(pi_cam.camera.get(3)), int(pi_cam.camera.get(4))))
